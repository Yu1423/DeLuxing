...
void deLuxing(GRBmodel *lp, double UB, int NClust, int beta1, int beta2, vector<int>& Idxdel, double Tolerance, bool Verbose){
	...
	cout << "\n|R| = " << NN << ", |R_1| = " << num_half << ", # of vars deleted = " << ncol_del0  << "." << endl;
	cout << "------------------------------------------------------------------------------------" << endl;
	...
	if(NCol <= beta1){
		...
		kmeans:kmeans_lloyd_parallel(data, means, clusters, parameters);
	}
	else{
...
Terminate:
	...
	cout << "Total time = " << getWallTime() - t << ", # of vars at termination = " << NCol << ", deleted " << (NN - NCol) *100.0 / NN << "% of the vars.\n\n" << endl;
}
